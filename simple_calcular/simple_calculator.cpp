/* 
 * Implement a simple 'four-function' calculator using stacks and queues.
 ...
 * -3 * -2 => 6
 * 1 + 2 + 3 * 4 => 15
 * Created on November 28, 2012, 4:11 PM
 */
...
// add to read, remove from front
int ifront=0, irear=0;
int icount=0;
void enqueue(int n){
    if(icount<100){
        queue_buffer[ifront+icount]=n;
        icount++;
    }
}
int dequeue(){
...
struct token_queue infix_to_postfix(struct token_queue * pqueue_infix) {
    ...
    p_expr_token ptoken;
    token_queue queue_postfix; // this is our output
    ...
    for(ptoken = dequeue(pqueue_infix); ptoken; ptoken = dequeue(pqueue_infix)){
        switch(ptoken->type){
            case OPERAND:
                ...
                break;
            case OPERATOR:
                // operators added to stack, after operators of higher precedence are
                // moved to queue
                while( stack_top &&
